...
### 系統目標
Ghote 旨在成為學生和終身學習者的智能學習夥伴，透過 AI 技術自動從學習資料中提取核心知識，並以多種形式呈現學習內容，包括重點筆記、選擇題、問答題及抽認卡。用戶可以建立 Project 管理不同科目或主題的學習資料，AI 會自動分析並生成各種學習材料。
...
4. **抽認卡**：50 張卡片，正面是問題，背面是答案

小明先閱讀重點筆記快速複習，然後使用選擇題自我測驗，最後用抽認卡進行記憶訓練。系統會記錄他的學習進度，標記困難的題目供日後複習。
...
3. **資料儲存**
   - App 解析 JSON 資料
   - App 透過 `Firestore SDK` 將生成的筆記、題目、抽認卡寫入 Cloud Firestore
   - 更新文件處理狀態為 `Completed`
...
#### Story: AI-US-04 生成抽認卡
- **故事**：作為一位學習者，我希望 AI 能生成抽認卡，以便進行快速記憶訓練。
- **註記**：
  - 卡片正面顯示問題，背面顯示答案
  - 支援翻轉動畫
  - 可標記為「已掌握」或「需複習」
...
#### Story: LEARN-US-02 抽認卡學習模式
- **故事**：作為一位學習者，我希望能使用抽認卡模式學習，以便高效記憶知識點。
- **註記**：
  - 支援左右滑動切換卡片
  - 點擊卡片翻轉查看答案
  - 可標記「已掌握」/「需複習」/「困難」
...
**FR-4.8** 系統應生成抽認卡，包含問題（正面）、答案（背面）、標籤、難度。
...
**FR-9.3** 系統應統計抽認卡的學習進度（已掌握/需複習/困難的比例）。
...
**NFR-1.6** 抽認卡翻轉動畫應保持 **60 FPS** 的流暢度。
